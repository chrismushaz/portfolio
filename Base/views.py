...
def contact(request):
    if request.method == "POST":
        ...
        if len(name) <= 1 or len(name) >= 30:
            return render(request, 'contact.html', {
                'error': 'Name should be greater than 2 and less than 30 characters.'
            })
        ...
        if len(phone) <= 9 or len(phone) >= 13:
            return render(request, 'contact.html', {
                'error': 'Invalid phone number. It should be 10 to 12 digits.'
            })
    ...
    return render(request, 'contact.html')  # fallback GET
